...
final class AutomatedReviewRequestService {
    ...
    init(defaults: UserDefaults = .standard) {
        ...
        storedActiveDuration = defaults.double(forKey: DefaultsKey.totalActiveDuration)
    }
    ...
    func requestReviewIfEligible(
        ...
        let completedCount = defaults.integer(forKey: DefaultsKey.completedMilestoneCount)
        ...
        saveActiveSegment(at: date)
        defaults.set(milestone + 1, forKey: DefaultsKey.completedMilestoneCount)
        defaults.set(
            (requestDates + [date]).map(\.timeIntervalSinceReferenceDate),
            forKey: DefaultsKey.requestDates
        )
        requestReview()
        activeSegmentStartedAt = date
    }
    ...
    private var storedRequestDates: [Date] {
        (defaults.array(forKey: DefaultsKey.requestDates) as? [NSNumber] ?? []).map {
    ...
    private func saveActiveSegment(at date: Date) {
        ...
        defaults.set(storedActiveDuration, forKey: DefaultsKey.totalActiveDuration)
        activeSegmentStartedAt = nil
        lastPeriodicSaveDate = date
    }
